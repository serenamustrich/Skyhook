...
pub trait ActiveKeyExchange: Send + Sync {
    ...
    /// Optionally computes a shared secret without consuming this key exchange.
    ///
    /// This is used by specialized ClientHello authentication schemes that must
    /// derive data before the TLS handshake consumes the key exchange. Normal
    /// implementations should keep the default `None`.
    fn dangerous_shared_secret_for_client_hello(
        &self,
        _peer_pub_key: &[u8],
    ) -> Option<Result<SharedSecret, Error>> {
        None
    }
