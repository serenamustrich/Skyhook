...
/// Provides a specialized ClientHello session id for protocols that authenticate
/// through TLS compatibility bytes.
pub trait DangerousClientHelloSessionIdProvider: fmt::Debug + Send + Sync {
    /// Returns the clear session id bytes that must be present while the initial
    /// ClientHello is encoded for authentication.
    fn plaintext_session_id(&self) -> [u8; 32];

    /// Returns the final session id bytes to send on the wire.
    fn seal_session_id(
        &self,
...
pub struct ClientConfig {
    ...
    /// Provides a custom TLS 1.3 compatibility session id sent in ClientHello.
    ///
    /// This is intentionally a sharp edge for protocols such as REALITY that
    /// authenticate through the legacy session id bytes. Normal clients should
    /// leave it as `None` so rustls generates fresh random bytes.
    pub dangerous_client_hello_session_id_provider:
        Option<Arc<dyn DangerousClientHelloSessionIdProvider>>,
...
pub(super) mod danger {
    ...
    impl DangerousClientConfig<'_> {
        ...
        /// Installs a custom TLS 1.3 ClientHello compatibility session id provider.
        pub fn set_client_hello_session_id_provider(
            &mut self,
            provider: Arc<dyn super::DangerousClientHelloSessionIdProvider>,
        ) {
            self.cfg.dangerous_client_hello_session_id_provider = Some(provider);
        }
    }
}
