...
fn example_config_parses() {
    let config: SuperConfig =
        serde_yaml::from_str(include_str!("../supercore.example.yaml")).expect("example config");
...
async fn dns_exchange_uses_tcp_framing_through_runtime() {
    ...
    config.dns.default_nameserver = vec![addr.to_string()];
    config.dns.nameserver = vec![addr.to_string()];
...
fn unique_test_path(name: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("supercore-{name}-{nanos}.json"))
}
